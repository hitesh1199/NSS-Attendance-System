...
router.post('/addEvent', async (req, res) => {
  ...
  try {
    ...
    const convertedStartDate = moment.tz(startDate, format, indiaTimeZone).toDate();
    ...
    let event = new Event({ eventName: name, startDate: convertedStartDate, endDate: convertedEndDate });
...
router.put('/updateEvent/:id', async (req, res) => {
  ...
  try {
    ...
    console.log("hello " + event.eventName + "  " + event.startDate + "  " + event.endDate);
    await event.save();
...
router.delete('/deleteEvent/:id', async (req, res) => {
  ...
  try {
    ...
    return res.json({ success: false, message: 'Error occurred while deleting event' });
  }
});
